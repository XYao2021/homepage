...
\entry{Graduate Research Assistant, EdgeAI Lab (Prof. Mingyue Ji)}{08/2021 -- present}
...
\meta{University of Florida \textperiodcentered\ Platform for the over-the-air experiments in [C1], [C2], [C4]}
\begin{itemize}
  \item Implemented the physical-layer software stack of the lab's wireless testbed in C++: modulation and
        demodulation, pulse-shaping and filtering, frame and symbol synchronization, channel encoding and
        decoding, and estimation and correction of carrier-frequency offset and phase offset.
  ...
  \item Deployed the group's learning algorithms (reinforcement-learning spectrum sharing, power allocation,
        ...
        experiments in [C1].
  \item Technologies: C++, Python, USRP/UHD, LoRa transceivers, Linux.
\end{itemize}
...
\begin{itemize}
  \item Developed and deployed the AWS-hosted web user interface through which remote users reserve and
        access the wireless testbeds.
  \item Designed the physical-layer (PHY) code paths for heterogeneous hardware, including USRP
        software-defined radios and LoRa devices, so that the same experiment can target different radios.
...
\begin{itemize}
  \item Derived optimization objectives with Lyapunov drift-plus-penalty (LDPP) functions under real-world
        constraints (e.g., power, interference, and queue stability), turning long-term constrained problems
        into per-slot decisions that a learning agent can act on.
  ...
  \item Proposed LDPP-MADDPG, a distributed power-allocation scheme for mmWave cellular networks in which
        each base station acts as a multi-agent DDPG agent guided by a Lyapunov drift-plus-penalty objective
        [C5]; an earlier MADDPG power-allocation study used measurement data from the POWDER platform [P4].
  ...
  \item Technologies: Python, PyTorch, reinforcement learning, Lyapunov optimization.
\end{itemize}
...
\begin{itemize}
  ...
  \item Designed the system architecture for jointly adapting LoRa PHY parameters (e.g., spreading factor,
        bandwidth, coding rate, and transmit power) on a per-slot basis.
...
\begin{itemize}
  \item Device-free indoor localization that fingerprints LoRa channel measurements collected with commodity
        LoRa radios, so that a target carrying no device can be localized from changes in the wireless channel.
  \item Designed the fingerprint data collection and the localization pipeline, and evaluated the system with
        commodity hardware in an indoor environment.
\end{itemize}
...
\begin{itemize}
  ...
  \item Evaluated the algorithm against existing decentralized learning methods under different compression
        levels, network topologies, and data-heterogeneity settings.
  \item Technologies: Python, PyTorch, optimization theory.
\end{itemize}
...
\begin{enumerate}[label={[C\arabic*]}]
  ...
  \item \me, A.\ Bhuyan, X.\ Zhang, and M.\ Ji, ``A Novel LDPP-MADDPG Approach for Distributed Power Allocation
        ...
        drift-plus-penalty (LDPP) objective with multi-agent deep deterministic policy gradient (MADDPG), each
        base station acting as an agent.}
...
\begin{itemize}
  ...
  \item \textbf{Machine learning:} reinforcement learning (Q-learning, DDPG, MADDPG, transformer-enhanced
        ...
        secure aggregation, distributed and parallel computing, PyTorch.
